...
function makeDingDingRobotHook(data, job) {
  var dingdingContent;
  if(data.exitCode == 0) {
    dingdingContent = `${job.trigger.message} ${job.project.name}:${job.ref.branch} ${job.type} is finished at ${data.time}，test_exitcode: ${data.exitCode}`;
  }
  else {
    dingdingContent = `${job.trigger.message} ${job.project.name}:${job.ref.branch} ${job.type} was failed at ${data.time}，test_exitcode: test error}`;
  }
  return {
    "msgtype": "text",
    "text": {
