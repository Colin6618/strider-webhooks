...
function makeDingDingRobotHook(data, job) {
  ...
  let jobMessage, mobile, atMobiles;
  if (regResult) {
    jobMessage = job.trigger.message.slice(0, regResult.index)
    mobile = regResult && regResult[0].trim();
    atMobiles = [].concat(mobile);
  } else {
    jobMessage = job.trigger.message;
    atMobiles = [];
  }
  if (data.exitCode == 0) {
